...
<div class="panel-body panel-body-nopadding">
	...
	<div class="form-group row"><label class="col-4 col-form-label">Discount Name</label><div class="col-8"><input name="name" class="form-control" required="required" id="name" type="text" value="{{$discount->name}}"></div></div>
	...
	<div class="form-group row">
	<label class="col-4 col-form-label">Discount amount </label><div class="col-8"><input name="amount" class="form-control" required="required" id="amount_1" type="text" value="{{$discount->amount}}" {{$discount->disc_type=='Open'?'disabled':''}}></div>	</div>
	<div class="form-group row">
...
<div class="col-sm-6" style="float:left">	
	...
	<div class="col-sm-4">
		...
		@foreach($ordTypes as $ordTyp)
			<div class="ckbox ckbox-primary">
			<input name="ord_typ_id[]" value="{{$ordTyp->id}}" id="echeckbox{{$ordTyp->id}}" type="checkbox" @if($discount->ord_typ_id && in_array($ordTyp->id,$discount->ord_typ_id)) checked @endif >
			<label for="echeckbox{{$ordTyp->id}}">{{$ordTyp->name}}</label>
		 </div>
		@endforeach
		@endif 
	</div>
